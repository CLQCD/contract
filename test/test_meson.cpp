...
void meson(void *correl, void *propag_a, void *propag_b, size_t volume, int gamma_ab, int gamma_dc)
{
  ...
  for (int i = 0; i < NITER; ++i) { meson_two_point(correl, propag_a, propag_b, volume, gamma_ab, gamma_dc); }
  ...
  printf("Time elapsed: %f ms, Bandwidth: %f GB/s\n", milliseconds / NITER,
         (2.0 * volume * 4 * 4 * 3 * 3 * 16 + volume * 16) / (1024 * 1024 * 1024) / (milliseconds / 1000.0 / NITER));
